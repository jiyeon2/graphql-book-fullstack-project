...
function SignUpRealForm() {
  ...
  const onSubmit = async (data: SignUpMutationVariables) => {
    ...
    return signUp({ variables: { signUpInput } })
      .then((res) => {
        if (res.data?.signUp) {
          toast({ title: '회원가입을 축하합니다', status: 'success' });
          history.push('/');
        } else {
          toast({
  ...
  return (
    <Stack as="form" spacing={4} onSubmit={handleSubmit(onSubmit)}>
      <FormControl isInvalid={!!errors.signUpInput?.email}>
        ...
        <Input
          type="email"
          placeholder="example@exmalsc.com"
...
export default function SignUpForm(): React.ReactElement {
  return (
    <Stack spacing={8} mx="auto" maxW="lg" py={12} px={6}>
      <Stack align="center">
        <Heading fontSize="4xl">지브리 명장명 프로젝트</Heading>
